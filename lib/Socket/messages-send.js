...
sendStatusMentions: async (jids, content) => {
...
// pastiin jids array, kalo bukan, ubah jadi array
if (!Array.isArray(jids)) jids = [jids]

const mentionedUsersContent = jids.map(jid => ({
tag: 'to',
attrs: { jid },
content: undefined,
}))
...
for (const jid of jids) {
let isPrivate = WABinary_1.isJidUser(jid)
if (isPrivate) {
allStatusJids.push(jid)
} else {
const group = await groupMetadata(jid)
allStatusJids.push(...group.participants.map(p => p.id))
}
}

await relayMessage(WABinary_1.STORIES_JID, media.message, {
messageId: media.key.id,
statusJidList: allStatusJids,
additionalNodes,
})
